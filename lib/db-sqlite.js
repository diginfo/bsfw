...
function libs(){
  try {
    var libname = 'node_sqlite3.node';
    var src = path.join(paths.abs,'nodelib',process.platform,libname);
    var tgt = path.join(paths.abs,libname);
    //cl('src:',src,'tgt:',tgt);
    if($.fs.existsSync(src) && !$.fs.existsSync(tgt)) $.fs.copyFileSync(src,tgt);
    const sqlite3 = require('sqlite3');
    
  } catch (err){
    ce(`ERR: Cannot copy ${libname}: ${err.message}`);
  }
}
