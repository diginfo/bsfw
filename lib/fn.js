...
function fileOlder(path,secs){
	try{return ((new Date - $.fs.statSync(path).mtime) /1000) > secs}
...
function jsonPut(fn,wdata,cb){
  jsonGet(fn,function(rdata){
    data = merge(rdata,wdata);
    $.fs.writeFile(fn, jsonString(data), function(err) {
      if(err) return cb({error:true,msg:err});
      else cb(data);
    })    
  })
}
...
function jsonGet(fn,cb){
  $.fs.readFile(fn, 'utf8',function(err,res){
    if(err) {
      ce(err);
      return cb({})
    }
    else return cb(jsonParse(res));
  });
}

function configGet(key,cb){
  jsonGet($.paths.config,function(data){
    if(cb) return cb(data[key]);
    key(data);  
  });
}

function configPut(tdata,cb=mex.cl){
  
  var keys = Object.keys(tdata);
  if(keys.length > 1) return ({error:true,msg:'Invalid Data'})
  var key = keys[0]; // MSDB
  jsonGet($.paths.config,function(data){
    var src = data[key];
    for(var k in tdata[key]){
      data[key][k] = tdata[key][k];  
    }
    
    //return mex.cl(JSON.stringify(data,null,2));
    $.fs.writeFile(confn, jsonString(data), function(err) {
      if(err) return cb({error:true,msg:err});
      else cb(data);
    });
  }) 
...
function encrypt(text){
  var cipher = _crypto.createCipher('aes-256-cbc',mex.salt)
...
function decrypt(text){
  var decipher = _crypto.createDecipher('aes-256-cbc',mex.salt)
  try{
